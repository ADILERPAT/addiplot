...
public class GlobalMembersAxis
{
	...
	public static e_autoscale load_range(AXIS_INDEX axis, double a, double b, e_autoscale autoscale)
	{
		...
		else if (GlobalMembersUtil.equals(GlobalMembersCommand.c_token, "to") == 0 && GlobalMembersUtil.equals(GlobalMembersCommand.c_token, ":") == 0)
		{
			if (GlobalMembersUtil.equals(GlobalMembersCommand.c_token, "*") != 0)
			{
				autoscale.setValue(autoscale.getValue() | e_autoscale.AUTOSCALE_MIN.getValue());
			...
			else
			{
				do
				{
					...
					do
					{
						if (((DefineConstants.FIRST_AXES) >= 0) && (axis_array[(DefineConstants.FIRST_AXES) + (axis.getValue())].is_timedata) && (GlobalMembersUtil.isstring(GlobalMembersCommand.c_token) != 0 || GlobalMembersUtil.type_udv(GlobalMembersCommand.c_token) == DATA_TYPES.STRING.getValue()))
						{
							tm tm = new tm();
		...
		if (GlobalMembersUtil.equals(GlobalMembersCommand.c_token, "]") == 0)
		{
			if (GlobalMembersUtil.equals(GlobalMembersCommand.c_token, "*") != 0)
			{
				autoscale.setValue(autoscale.getValue() | e_autoscale.AUTOSCALE_MAX.getValue());
				GlobalMembersCommand.c_token++;
			}
			else
			{
		...
		if ((autoscale.getValue() & e_autoscale.AUTOSCALE_BOTH.getValue()) == e_autoscale.AUTOSCALE_NONE.getValue())
		{
			if (b < a)
			{
				double temp = a;
	...
	public static void axis_checked_extend_empty_range(AXIS_INDEX axis, String mesg)
	{
		...
		{
			/* empty range */
			if (axis_array[axis.getValue()].autoscale.getValue() != 0)
			{
				 ...
				 * autoscaling, too: */
				if (axis_array[axis.getValue()].autoscale & e_autoscale.AUTOSCALE_MIN.getValue() != 0)
					axis_array[axis.getValue()].min -= widen;
				if (axis_array[axis.getValue()].autoscale & e_autoscale.AUTOSCALE_MAX.getValue() != 0)
	...
	public static double quantize_normal_tics(double arg, int guide)
	{
		...
		double power = GlobalMembersAxis.dbl_raise(10.0, (int) Math.floor(Math.log10(arg)));
	...
	{
		double tic = 0;
		axis this_Renamed = axis_array[axis.getValue()];
		 ...
		 * automatic tics */
		boolean autoextend_min = ((this_Renamed.autoscale.getValue() & e_autoscale.AUTOSCALE_MIN.getValue()) != 0) && ((this_Renamed.autoscale.getValue() & e_autoscale.AUTOSCALE_FIXMIN.getValue()) == 0);
		boolean autoextend_max = ((this_Renamed.autoscale.getValue() & e_autoscale.AUTOSCALE_MAX.getValue()) != 0) && ((this_Renamed.autoscale.getValue() & e_autoscale.AUTOSCALE_FIXMAX.getValue()) == 0);
	 ...
	 * "non-running", below. */
	public static void axis_output_tics(AXIS_INDEX axis, /* axis number we're dealing with */
			...
			tic_callback callback) /* tic-drawing callback function */
	{
		...
		if (axis_array[axis.getValue()].ticmode != 0)
		{
			...
			if (axis_array[axis.getValue()].tic_rotate == DefineConstants.TEXT_VERTICAL && term.text_angle(DefineConstants.TEXT_VERTICAL))
			{
			...
			else if (axis_array[axis.getValue()].tic_rotate != 0 && term.text_angle(axis_array[axis.getValue()].tic_rotate))
			{
				switch (axis)
				{
	...
	public static void axis_draw_2d_zeroaxis(AXIS_INDEX axis, AXIS_INDEX crossaxis)
	{
		...
		if (GlobalMembersAxis.axis_position_zeroaxis(crossaxis) && (this_Renamed.zeroaxis.l_type > DefineConstants.LT_NODRAW))
		{
			...
			if ((axis.getValue() % DefineConstants.SECOND_AXES) == AXIS_INDEX.FIRST_X_AXIS.getValue())
			{
				term.move(this_Renamed.term_lower, axis_array[crossaxis.getValue()].term_zero);
				term.vector(this_Renamed.term_upper, axis_array[crossaxis.getValue()].term_zero);
			}
			else
			{
				term.move(axis_array[crossaxis.getValue()].term_zero, this_Renamed.term_lower);
				term.vector(axis_array[crossaxis.getValue()].term_zero, this_Renamed.term_upper);
			}
		}
	}
	...
	public static void add_tic_user(AXIS_INDEX axis, String label, double position, int level)
	{
		...
		listhead.position = -DefineConstants.DBL_MAX;
   ...
   Return 0 on wrong range, otherwise 1.
	...
	{
		if ((axis_array[AXIS_INDEX.COLOR_AXIS.getValue()].set_autoscale.getValue() & ((e_autoscale.AUTOSCALE_MIN.getValue() != 0) ? 1 : 0)) != 0)
		{
		...
		if ((axis_array[AXIS_INDEX.COLOR_AXIS.getValue()].set_autoscale.getValue() & ((e_autoscale.AUTOSCALE_MAX.getValue() != 0) ? 1 : 0)) != 0)
		{
			/* -VERYLARGE according to AXIS_INI3D */
