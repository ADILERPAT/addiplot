...
public class GlobalMembersGraphics
{
	...
	public static void do_plot(LinkedList<curve_points> first_plot, int pcount)
	{
		...
		GlobalMembersPm3d.set_plot_with_palette(0, MODE_PLOT_TYPE.MODE_PLOT.getValue()); // EAM FIXME - 1st parameter is a dummy
		...
		GlobalMembersGraphics.place_objects(GlobalMembersGadgets.first_object, -1, 2, null);
		...
		if (GlobalMembersAxis.axis_array[AXIS_INDEX.FIRST_Y_AXIS.getValue()].label.text != null)
		{
			...
			GlobalMembersGadgets.apply_pm3dcolor(GlobalMembersAxis.axis_array[AXIS_INDEX.FIRST_Y_AXIS.getValue()].label.textcolor);
			/* we worked out x-posn in boundary() */
			if (term.text_angle(GlobalMembersAxis.axis_array[AXIS_INDEX.FIRST_Y_AXIS.getValue()].label.rotate) != 0)
			{
				double tmpx;
				double tmpy;
				int x;
				...
				y = (int) ((GlobalMembersGadgets.plot_bounds.ytop + GlobalMembersGadgets.plot_bounds.ybot) / 2 + tmpy);
			...
			GlobalMembersGadgets.reset_textcolor(GlobalMembersAxis.axis_array[AXIS_INDEX.FIRST_Y_AXIS.getValue()].label.textcolor);
		...
		if (GlobalMembersAxis.axis_array[AXIS_INDEX.SECOND_Y_AXIS.getValue()].label.text != null)
		{
			...
			GlobalMembersGadgets.apply_pm3dcolor(GlobalMembersAxis.axis_array[AXIS_INDEX.SECOND_Y_AXIS.getValue()].label.textcolor);
			/* we worked out coordinates in boundary() */
			if (term.text_angle(GlobalMembersAxis.axis_array[AXIS_INDEX.SECOND_Y_AXIS.getValue()].label.rotate) != 0)
			{
				double tmpx;
				double tmpy;
				int x;
			...
			GlobalMembersGadgets.reset_textcolor(GlobalMembersAxis.axis_array[AXIS_INDEX.SECOND_Y_AXIS.getValue()].label.textcolor);
			GlobalMembersTerm.ignore_enhanced(false);
		}
		...
		if (GlobalMembersAxis.axis_array[AXIS_INDEX.FIRST_X_AXIS.getValue()].label.text != null)
		{
			...
			x = (int) ((GlobalMembersGadgets.plot_bounds.xright + GlobalMembersGadgets.plot_bounds.xleft) / 2 + tmpx);
		...
		if (GlobalMembersGadgets.timelabel.text != null)
		{
			...
			if (GlobalMembersGadgets.timelabel_rotate != 0 && (term.text_angle(DefineConstants.TEXT_VERTICAL) != 0))
			{
				x += term.v_char / 2; // HBB
		...
		if (GlobalMembersPm3d.is_plot_with_colorbox() && GlobalMembersGadgets.color_box.layer == DefineConstants.LAYER_BACK)
			GlobalMembersColor.draw_color_smooth_box(MODE_PLOT_TYPE.MODE_PLOT.getValue());
		...
		if (lkey) // may have been cancelled if something went wrong
		{
			...
			if (key.title != null)
			{
				...
				if (((term.flags & DefineConstants.TERM_ENHANCED_TEXT) != 0) && StringFunctions.strChr(key.title,'^'))
					extra_height += 0.51;
				...
				if ((term.flags & DefineConstants.TERM_ENHANCED_TEXT) && StringFunctions.strChr(key.title,'_'))
					extra_height += 0.3;
			...
			if (key.box.l_type > DefineConstants.LT_NODRAW)
			{
				...
				if (term.flags & DefineConstants.TERM_CAN_CLIP != 0)
					GlobalMembersGadgets.clip_area = null;
				else
   ...
   label. */
	...
	private static gpiPoint do_polygon_corners = null;
	//C++ TO JAVA CONVERTER NOTE: This was formerly a static local variable declaration (not allowed in Java):
	private static gpiPoint do_polygon_clpcorn = null;
	...
	public static coordinate stackheight = null; // top of previous row
