...
//C++ TO JAVA CONVERTER NOTE: 'extern' variable declarations are not required in Java:
	...
	public static void do_3dplot(surface_points plots, int pcount, int quick)
	{
		...
		int surface;
		surface_points this_plot = DefineConstants.NULL;
		int xl;
		int yl;
		...
		{
		if (GlobalMembersAxis.axis_array[GlobalMembersAxis.z_axis.getValue()].range_flags & DefineConstants.RANGE_REVERSE != 0)
		{
		...
		GlobalMembersAxis.axis_set_graphical_range(AXIS_INDEX.FIRST_Z_AXIS, floor_z, ceiling_z);
		...
		GlobalMembersGraphics.place_objects(GlobalMembersGadgets.first_object, -1, 3, DefineConstants.NULL);
...
//ORIGINAL LINE: term_apply_lp_properties(&border_lp);
	...
	///#undef DEFAULT_Y_DISTANCE
		...
		x = t.v_char + tmpx;
		y = GlobalMembersGadgets.timelabel_bottom != 0 ? GlobalMembersGadgets.yoffset * GlobalMembersAxis.axis_array[GlobalMembersAxis.y_axis.getValue()].max + tmpy + t.v_char : GlobalMembersGadgets.plot_bounds.ytop + tmpy - t.v_char;
...
//C++ TO JAVA CONVERTER TODO TASK: There are no simple equivalents to function pointers in Java:
	//		(*t.text_angle)(0);
		...
		GlobalMembersColor.draw_color_smooth_box(MODE_PLOT_TYPE.MODE_SPLOT);
...
//ORIGINAL LINE: place_objects(first_object, 0, 3, &plot_bounds);
	...
	///#endif // not LITE
		...
		if (key.region != en_key_region.GPKEY_AUTO_INTERIOR_LRTBC && key.margin == en_key_ext_region.GPKEY_BMARGIN)
		{
			...
			yl = GlobalMembersGadgets.yoffset * t.ymax + (key_rows) * key_entry_height + (ktitle_lines + 2) * t.v_char;
			}
		...
		else
		{
			...
			{
			yl = GlobalMembersGadgets.plot_bounds.ybot + t.v_tic + key_entry_height * key_rows + ktitle_lines * t.v_char;
			}
...
//C++ TO JAVA CONVERTER TODO TASK: There are no simple equivalents to function pointers in Java:
	//		(*t.linetype)(DefineConstants.LT_BLACK);
			...
			if (((int)draw_contour) != 0 && this_plot.contours != DefineConstants.NULL)
			{
 ...
 */
	...
	/* Set by 'set view map': */
	public static int splot_map = false;
 ...
 */
	public static void cntr3d_impulses(gnuplot_contours cntr, lp_style_type lp)
	{
		...
		if (draw_contour & en_contour_placement.CONTOUR_SRF != 0)
		{
